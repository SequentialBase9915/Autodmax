...
import math
##############################################################################
# Below initializes pygame for Moonlight input and defines functions for 
# translating XBOX 360 commands from Moonlight to appropriate Switch Hex.
##############################################################################

ser = serial.Serial(port='COM3', baudrate=19200,timeout=1)
...
# Edited to determine which stick to use via input (defined in stick_move)
def stick_angle(angle, intensity, stick):
    if stick == 0:
...
##############################################################################
# Needs to run to ensure connectivity upon disconnect of physical hardware
##############################################################################
def serial_connect():
    if not sync(): 
